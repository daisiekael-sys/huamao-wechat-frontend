...
async function openPublicProfile(userId) {
  try {
    ...
    const activities = p.activities.length ? `<div class="mt-4"><h4 class="text-xs font-semibold text-gray-500 mb-2">发起的公开活动</h4><div class="space-y-2">${p.activities.map(a => `<button class="w-full text-left bg-gray-50 rounded-xl p-3" onclick="closeModal();openActivity('${a.id}')"><b class="text-sm">${esc(a.title)}</b><span class="block text-xs text-gray-400 mt-1">${fmtDate(a.start_at)}${a.location ? ` · ${esc(a.location)}` : ''}</span></button>`).join('')}</div></div>` : '';
    openModal(`<div class="flex items-center gap-3"><div class="avatar w-12 h-12" style="background:${p.avatar_url ? 'transparent' : p.avatar_color}">${p.avatar_url ? `<img src="${esc(p.avatar_url)}" class="w-full h-full rounded-full object-cover">` : esc(p.display_name[0])}</div><div><h2 class="text-xl font-bold">${esc(p.display_name)}</h2><p class="text-xs text-gray-400">${p.followers} 人关注 · ${p.confirmed_count} 条已确认成果</p></div></div><p class="welcome-copy mt-4">${esc(p.bio || '这个人还没有填写介绍。')}</p>${p.focus ? `<dl class="task-agreement"><dt>近期在做</dt><dd>${esc(p.focus)}</dd>${p.need ? `<dt>正在寻找</dt><dd>${esc(p.need)}</dd>` : ''}</dl>` : ''}${blocks}${activities}<div class="mt-4 flex gap-2">${self ? '<button class="cat-btn px-4 py-2" onclick="closeModal();openProfile()">编辑我的主页</button>' : S.user ? `<button class="cat-btn px-4 py-2" onclick="followUserProfile('${p.id}',${p.followed ? 'true' : 'false'})">${p.followed ? '取消关注' : '关注这个人'}</button>` : '<button class="cat-btn px-4 py-2" onclick="closeModal();showView(\'auth\')">登录后关注</button>'}<button class="tab-btn" onclick="closeModal()">关闭</button></div>`);
  } catch (e) { toast(e.message, 'err'); }
}
